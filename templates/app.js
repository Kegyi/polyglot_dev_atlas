(function () {
    ...
    function renderHomeView() {
        ...
        selected.forEach(function (langKey) {
            ...
            var list = '<ul>' + points.map(function (p) {
                return '<li>' + escapeHtml(p) + '</li>';
            }).join('') + '</ul>';
            rows += '<tr><td>' + escapeHtml(LANG_LABELS[langKey] || langKey) + '</td><td>' + list + '</td></tr>';
        });

        if (!rows) {
            rows = '<tr><td>None</td><td>Select a language above to show language-specific features.</td></tr>';
        }

        var dynamicSection = ''
            + '<div class="doc-table-wrap"><table class="doc-table">'
            + '<thead><tr><th>Language</th><th>Detailed language-specific features</th></tr></thead>'
